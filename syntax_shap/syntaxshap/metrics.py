...
from syntax_shap.syntaxshap.maskers import Text
...
def replace_tokens_randomly_full(
...
def get_scores(
    ...
    inputs_randomized = replace_tokens_randomly_full(results["token_ids"].apply(lambda x: x[1:]).tolist(), masks, pipeline.tokenizer)
    ...
    # delete large probabilities array to avoid OOM error
    del probs_keep
    gc.collect()
    ...
    for i, str_input in enumerate(results["input"]):
        if masks[i] is None:
            ...
            new_str_input = inputs_randomized[i] #replace_token_randomly(str_input, mask, pipeline.tokenizer)
...
def save_scores(args, scores, i):
    ...
    filename += f"{args.dataset}_{args.model_name}_{args.algorithm}_{args.seed}_{i}.pkl"
