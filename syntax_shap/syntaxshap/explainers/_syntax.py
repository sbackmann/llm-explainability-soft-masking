...
from syntax_shap.syntaxshap.utils import (
    MaskedModel,
    OpChain,
...
class SyntaxExplainer(Explainer):
    ...
    def explain_row(self, *row_args, max_evals, main_effects, error_bounds, batch_size, outputs, silent):
        ...
        if 'syntax' in self.algorithm:
            ...
            dependency_dt, df_words, df_tokens, pos_token_to_word = get_token_dependency_tree(sentence=row_args[0], tokenizer=self.masker.tokenizer)
        else:
            dependency_dt = None
        #TODO
        causal_ordering_error = self.compute_shapley_values(M, algorithm=self.algorithm, weighted=self.weighted, dependency_dt=dependency_dt)
        if causal_ordering_error is not None:
            return causal_ordering_error, dependency_dt, df_words, df_tokens, pos_token_to_word
