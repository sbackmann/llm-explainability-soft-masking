...
import syntax_shap.syntaxshap.maskers as maskers
import syntax_shap.syntaxshap.models as models
import numpy as np
import pandas as pd
import scipy.sparse
from syntax_shap.syntaxshap._explanation import Explanation
from syntax_shap.syntaxshap._serializable import Deserializer, Serializable, Serializer
from syntax_shap.syntaxshap.maskers import Masker
from syntax_shap.syntaxshap.models import Model
from syntax_shap.syntaxshap.utils import safe_isinstance, show_progress
from syntax_shap.syntaxshap.utils._exceptions import InvalidAlgorithmError
from syntax_shap.syntaxshap.utils.transformers import is_transformers_lm
...
class Explainer(Serializable):
    ...
    def __call__(self, *args, max_evals="auto", main_effects=False, error_bounds=False, batch_size="auto",
        ...
        for row_args in show_progress(zip(*args), num_rows, self.__class__.__name__+" explainer", silent):
            ...
            if not isinstance(row_result, dict):
                error_sentences.append({
                    "sentence": row_args,
                    "causal_ordering": row_result[0],
                    "dependency_dt": row_result[1],
                    "df_words": row_result[2],
                    "df_tokens": row_result[3],
                    "pos_token_to_word": row_result[4]})
                continue
        ...
        with open("./non_error_sentences.pkl", "wb") as f:
            pkl.dump(error_sentences, f)
